...
class VideoUploadCoordinatorTest {
    @Test
    void expiresIncompleteVideoSessionsWithoutWaitingForAnotherPacket() {
        Player player = player();
        ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(1); when(films.maximumVideoFrames(camera, 100)).thenReturn(1); when(films.maximumVideoFps(camera, 10)).thenReturn(10); when(films.consume(camera, 1)).thenReturn(true);
        VideoUploadCoordinator coordinator = new VideoUploadCoordinator(PluginSettings.from(Map.of()), films, (ignored, packet) -> { }, (ignored, session, metadata) -> { }, ignored -> { });
    ...
    void beginChargesEveryFinalFrameTileAndGrantsConfiguredChunkRate() {
        Player player = player();
        ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(4); when(films.maximumVideoFrames(camera, 100)).thenReturn(100); when(films.maximumVideoFps(camera, 10)).thenReturn(10); when(films.consume(camera, 240)).thenReturn(true);
    ...
    @Test
    void rejectsVideoThatExceedsTheConfiguredActiveUploadMemoryBudget() {
        Player player = player(); ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(1); when(films.maximumVideoFrames(camera, 100)).thenReturn(1); when(films.maximumVideoFps(camera, 10)).thenReturn(10);
        List<CameraPacket> sent = new ArrayList<>();
        VideoUploadCoordinator coordinator = new VideoUploadCoordinator(PluginSettings.from(Map.of("video.max-active-upload-bytes", 16_384L)), films,
    ...
    void rejectsFpsAboveTheHeldCamerasComponentLimitBeforeChargingFilm() {
        Player player = player();
        ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(4); when(films.maximumVideoFrames(camera, 100)).thenReturn(100);
    ...
    void discardsThePriorShutterHintWhenVideoBeginOwnsItsMetadata() {
        Player player = player(); ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(1); when(films.maximumVideoFrames(camera, 100)).thenReturn(100); when(films.maximumVideoFps(camera, 10)).thenReturn(10); when(films.consume(camera, 1)).thenReturn(true);
    ...
    void rejectsAFrameRateThatDoesNotAlignWithServerTicks() {
        Player player = player(); ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(1); when(films.maximumVideoFrames(camera, 100)).thenReturn(100); when(films.maximumVideoFps(camera, 10)).thenReturn(10);
        List<CameraPacket> sent = new ArrayList<>();
        VideoUploadCoordinator coordinator = new VideoUploadCoordinator(PluginSettings.from(Map.of()), films, (ignored, packet) -> sent.add(packet), (ignored, session, metadata) -> { }, ignored -> { });
    ...
    void rejectsSecondChunkInsideConfiguredOneChunkWindow() {
        ...
        coordinator.handle(player, new Packets.VideoPreviewChunk(token, 0, new byte[8_192]));
        coordinator.handle(player, new Packets.VideoPreviewChunk(token, 8_192, new byte[8_192]));
    ...
    @Test
    void acceptsClientPreviewBeforeCompletingTheVideo() {
        Player player = player();
        ItemStack camera = mock(ItemStack.class); CameraFilmService films = mock(CameraFilmService.class);
        when(films.heldCamera(player)).thenReturn(camera); when(films.supportsVideo(camera)).thenReturn(true); when(films.maximumVideoGridSize(camera, 4)).thenReturn(1); when(films.maximumVideoFrames(camera, 100)).thenReturn(1); when(films.maximumVideoFps(camera, 10)).thenReturn(10); when(films.consume(camera, 1)).thenReturn(true);
        List<CameraPacket> sent = new ArrayList<>();
        AtomicReference<dev.tobyscamera.common.upload.VideoUploadSession> completed = new AtomicReference<>();
        VideoUploadCoordinator coordinator = new VideoUploadCoordinator(PluginSettings.from(Map.of()), films, (ignored, packet) -> sent.add(packet), (ignored, session, metadata) -> completed.set(session), ignored -> { });

        coordinator.handle(player, new Packets.VideoBegin(1, 1, 1, 1));
        UUID token = ((Packets.VideoGranted) sent.getFirst()).token();
        coordinator.handle(player, new Packets.VideoPreviewChunk(token, 0, new byte[8_192]));
        coordinator.handle(player, new Packets.VideoPreviewChunk(token, 8_192, filled((byte) 31, 8_192)));
        ...
        assertEquals(31, Byte.toUnsignedInt(completed.get().previewPixels()[8_192]));
    }
