...
public final class ViewfinderOverlay {
    ...
    static PreflightReadout preflightReadout(int side, int maximum, AspectRatio ratio, int remainingFilm) {
        ...
        return new PreflightReadout("%dx%d maps".formatted(layout.gridWidth(), layout.gridHeight()),
                ...
                remainingFilm < 0 ? "max %dx%d".formatted(maximum, maximum)
                        : "film %d \u00b7 %d shots \u00b7 max %dx%d".formatted(
                                Math.max(0, remainingFilm), Math.max(0, remainingFilm) / maps, maximum, maximum));
    }
    ...
    private void drawCameraHud(GuiGraphics graphics, Minecraft minecraft, int left, int top, int width, int height) {
        ...
        graphics.drawString(minecraft.font, Component.literal(preflight.cost()), layout.safeLeft(), layout.gridTop(), HUD_MUTED_TEXT, true);
    }
