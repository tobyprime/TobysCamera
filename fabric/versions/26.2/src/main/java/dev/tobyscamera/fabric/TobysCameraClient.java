...
public final class TobysCameraClient implements ClientModInitializer {
    ...
    public static void captureWorldBeforeHand(net.minecraft.client.Minecraft client) {
        ...
        Screenshot.takeScreenshot(client.gameRenderer.mainRenderTarget(), nativeImage -> PHOTO_PROCESSOR_EXECUTOR.execute(() -> {
    ...
    public static boolean closeViewfinder() {
        if (net.minecraft.client.Minecraft.getInstance().gui.screen() instanceof ViewfinderControlsScreen) {
    ...
    private static void toggleCompositionEditor(net.minecraft.client.Minecraft client) {
        ...
        if (client.gui.screen() instanceof ViewfinderControlsScreen) client.gui.setScreen(null);
        else client.gui.setScreen(new ViewfinderControlsScreen(VIEWFINDER, COMPOSITION_KEY, TobysCameraClient::heldCameraGridSize));
    }
    ...
    private static boolean shutterInputAllowed() {
        return !(net.minecraft.client.Minecraft.getInstance().gui.screen() instanceof ViewfinderControlsScreen);
    }
    ...
    private static void openPreview(net.minecraft.client.Minecraft client, CapturedFrame frame) {
        ...
        client.gui.setScreen(new PreviewScreen(frame, VIEWFINDER.printSize(), VIEWFINDER.publicAddress(), VIEWFINDER.publicPhotographer(), VIEWFINDER.publicCapturedTime(),
                (photo, presentation) -> { if (VIEWFINDER.beginUpload() && !UPLOADS.confirm(photo.photo(), photo.bagPreview(), presentation)) VIEWFINDER.retake(); client.gui.setScreen(null); },
                () -> { VIEWFINDER.retake(); client.gui.setScreen(null); }, VIEWFINDER::setPublicAddress, VIEWFINDER::setPublicPhotographer, VIEWFINDER::setPublicCapturedTime));
    }
